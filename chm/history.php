...
if (isset($_SESSION['id'])) {
    $sql = "SELECT * FROM church_mem WHERE CMID = ?";
...
function getTableByUsername($conn, $username)
{
    ...
    foreach ($tables as $table) {
        ...
        if ($result->num_rows > 0) {
            $row = $result->fetch_assoc();
            return ["table" => $table, "user_id" => $row["CMID"]];
        }
    }
    return null;
}
...
function fetchUserEvents($conn, $username)
{
    ...
    $events_query = "SELECT APID, event_name, category, start_date, end_date, venue, reg_fee, status 
                     FROM appointment 
                     WHERE event_by = ?";
